...
SHOW_EMPIRICAL_CONDITIONAL_CURVE = True
...
def plot_empirical_copula(u_vals, v_vals, title="Pseudo-osservazioni della copula empirica (u,v)"):
...
def plot_conditional_mean_empirical_copula(u_vals, v_vals, bandwidth=0.05):
    u_grid = np.linspace(0.02, 0.98, 200)
    ...
    def weighted_quantile(values, quantile, sample_weight):
        ...
        return np.interp(quantile, normalized, values_sorted)

    # Stima non parametrica della media condizionata con kernel gaussiano (Nadaraya-Watson).
    diff = (u_grid[:, None] - u_vals[None, :]) / bandwidth
    weights = np.exp(-0.5 * (diff ** 2))
    denom = np.clip(weights.sum(axis=1), 1e-12, None)
    m_hat = (weights @ v_vals) / denom
    q05 = np.array([weighted_quantile(v_vals, 0.05, w) for w in weights])
    q50 = np.array([weighted_quantile(v_vals, 0.50, w) for w in weights])
    q95 = np.array([weighted_quantile(v_vals, 0.95, w) for w in weights])

    slope, intercept = np.polyfit(u_vals, v_vals, 1)
    linear_line = slope * u_grid + intercept

    fig, ax = plt.subplots(figsize=(8, 6))
    ax.scatter(u_vals, v_vals, alpha=0.25, s=10, color="steelblue", label="Pseudo-osservazioni")
    ax.plot(u_grid, linear_line, color="red", lw=2, linestyle="--", label=f"Spearman (retta sui rank, rho~{slope:.3f})")
    ax.plot(u_grid, m_hat, color="darkorange", lw=2.5, label=f"Media condizionata empirica di V dato U (smoothing={bandwidth:.2f})")
    ax.plot(u_grid, q50, color="green", lw=2, linestyle="-.", label="Quantile condizionato 50%")
    ...
    ax.set_xlabel("u")
    ax.set_ylabel("v")
    ax.set_title(f"Copula empirica: media e quantili condizionati {waahid} / {ithnaan}")
    ax.legend()
...
def gaussian_cdf(u_vals, v_vals, rho):
    ...
    c_vals = np.asarray(
        sc.stats.multivariate_normal.cdf(points, mean=np.zeros(2), cov=cov),
        dtype=float
    )
...
def rectangle_prob_from_cdf(cdf_func, low, high, **params):
    ...
    def cdf_at_point(u_point, v_point):
        out = cdf_func(u_point, v_point, **params)
        return float(np.asarray(out, dtype=float).reshape(-1)[0])

    c_hh = cdf_at_point(high_arr, high_arr)
    c_lh = cdf_at_point(low_arr, high_arr)
    c_hl = cdf_at_point(high_arr, low_arr)
    c_ll = cdf_at_point(low_arr, low_arr)
